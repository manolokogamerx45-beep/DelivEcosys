...
// Mock DocumentSnapshot class to bypass Firebase when useMockMode is active
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget _buildTextField({
    ...
    return TextField(
      ...
      style: TextStyle(color: isDark ? Colors.white : const Color(0xFF0F172A), fontSize: 14), // Letra adaptable
      decoration: InputDecoration(
        hintText: hint,
        hintStyle: const TextStyle(color: Color(0xFF94A3B8)),
        ...
        filled: true,
        fillColor: isDark ? const Color(0xFF334155) : const Color(0xFFF1F5F9), // Fondo adaptable
        border: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12),
          borderSide: BorderSide(color: isDark ? const Color(0xFF475569) : const Color(0xFFE2E8F0)),
        ),
        enabledBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12),
          borderSide: BorderSide(color: isDark ? const Color(0xFF475569) : const Color(0xFFE2E8F0)),
    ...
    return _driverFutureCache.putIfAbsent(
      ...
      () {
        if (AuthService.useMockMode) {
          final data = FirestoreService.mockDrivers[driverId];
          return Future.value(MockDocumentSnapshot(driverId, data));
        }
        return FirebaseFirestore.instance.collection('drivers').doc(driverId).get();
      },
    );
  }
            ...
            return AlertDialog(
              ...
              actions: [
                FilledButton(
                  onPressed: () async {
                    if (AuthService.useMockMode) {
                      if (FirestoreService.mockOrders.containsKey(order.id)) {
                        FirestoreService.mockOrders[order.id]!['isRated'] = true;
                      }
                    } else {
                ...
                Flexible(
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      icon: Icon(Icons.keyboard_arrow_down, color: isDark ? Colors.white70 : Colors.black54, size: 20),
                      style: TextStyle(
                        fontWeight: FontWeight.bold, 
                        color: isDark ? Colors.white : Colors.black87, 
                        fontSize: 13,
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: isDark ? const Color(0xFF60A5FA).withOpacity(0.15) : const Color(0xFF2563EB).withOpacity(0.1),
                  blurRadius: 24,
                  spreadRadius: 4,
                ),
              ],
            ),
            child: Icon(Icons.inventory_2_outlined, color: isDark ? const Color(0xFF60A5FA) : const Color(0xFF2563EB), size: 48),
          ),
      ...
      children: [
        ...
        StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance
              .collection('watches')
              .where('clientId', isEqualTo: widget.appUser.uid)
              .snapshots(),
          builder: (context, snapshot) {
            final isDark = Theme.of(context).brightness == Brightness.dark;
            final watches = snapshot.data?.docs ?? [];
            final isLinked = watches.isNotEmpty;
            final watchId = isLinked ? watches.first.id : null;
              ...
              children: [
                ...
                Card(
                  ...
                  child: ListTile(
                    leading: Icon(
                      Icons.watch_rounded,
                      color: isLinked ? const Color(0xFF10B981) : const Color(0xFF2563EB),
                    ),
                    title: Text(
                      isLinked ? 'Reloj Vinculado' : 'Vincular Smartwatch',
                      style: TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.bold,
                        color: isDark ? Colors.white : Colors.black87,
                    ...
                    subtitle: Text(
                      isLinked ? 'Dispositivo ID: $watchId' : 'Sincroniza tus notificaciones en tu Wear OS',
                      style: TextStyle(
                        fontSize: 11,
                        color: isDark ? Colors.white70 : Colors.black54,
                      ),
                    ...
                    trailing: isLinked
                        ? IconButton(
                            ...
                            onPressed: () async {
                              // Desvincular reloj
                              await FirebaseFirestore.instance.collection('watches').doc(watchId!).update({
                                'clientId': '',
                                'status': 'unpaired',
                              });
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Smartwatch desvinculado con éxito.')),
                                );
                              }
                            },
                          )
                    ...
                    onTap: isLinked
                        ...
                        : () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => _WatchScannerScreen(
                                  onScanned: (scannedWatchId) async {
                                    final firestoreService = Provider.of<FirestoreService>(context, listen: false);
                                    try {
                                      await firestoreService.pairWatch(scannedWatchId, widget.appUser.uid);
                                      if (context.mounted) {
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text('¡Smartwatch $scannedWatchId vinculado con éxito!'),
                                            backgroundColor: const Color(0xFF10B981),
                                          ),
                                        );
                                      }
                                      ...
                                      if (context.mounted) {
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text('Error al vincular: $e. Revisa las reglas de seguridad de Firestore.'),
...
class _RiderDashboardScreenState extends State<RiderDashboardScreen> {
  ...
  Future<void> _initializeRiderLocationAndNotify() async {
    ...
    try {
      ...
      if (AuthService.useMockMode) {
        if (FirestoreService.mockDrivers.containsKey(widget.appUser.uid)) {
          FirestoreService.mockDrivers[widget.appUser.uid]!['latitude'] = position.latitude;
          FirestoreService.mockDrivers[widget.appUser.uid]!['longitude'] = position.longitude;
  ...
  Widget _buildRiderDeliveriesTab(FirestoreService firestoreService) {
    return StreamBuilder<DocumentSnapshot>(
      stream: AuthService.useMockMode
          ? FirestoreService.mockDriversStream.map((list) {
              final data = FirestoreService.mockDrivers[widget.appUser.uid];
              return MockDocumentSnapshot(widget.appUser.uid, data);
            })
          : FirebaseFirestore.instance.collection('drivers').doc(widget.appUser.uid).snapshots(),
              ...
              ListTile(
                ...
                trailing: DropdownButton<String>(
                  ...
                  onChanged: (String? newValue) async {
                    if (newValue != null) {
                      if (AuthService.useMockMode) {
                        if (FirestoreService.mockDrivers.containsKey(driver.uid)) {
                          FirestoreService.mockDrivers[driver.uid]!['vehicle'] = newValue;
                          FirestoreService.notifyMockDrivers();
                        }
                        FirestoreService.mockOrders.forEach((key, val) {
                          if (val['driverId'] == driver.uid && ['accepted', 'in_transit', 'arrived'].contains(val['status'])) {
                            val['driverVehicle'] = newValue;
                          }
                        });
                      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.cloud_download_rounded, color: Color(0xFF3B82F6), size: 20),
            tooltip: 'Cargar Pedidos de Prueba',
            onPressed: () async {
              try {
                final firestoreService = Provider.of<FirestoreService>(context, listen: false);
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('✅ Base de datos cargada con pedidos de prueba con éxito'),
                      backgroundColor: Colors.green,
                    ),
                  );
                }
...
class _DriverProfileScreenState extends State<DriverProfileScreen> {
  ...
  Future<void> _loadDriverData() async {
    try {
      if (AuthService.useMockMode) {
        final data = FirestoreService.mockDrivers[widget.driverId];
        if (data != null) {
          _phone = data['phone'] ?? '';
        ...
        setState(() {
          ...
          _avgRating = 4.8;
          _comments = [
            {'client': 'Cliente Demo', 'rating': 5, 'comment': '¡Muy rápido y amable!'},
            {'client': 'Ana P.', 'rating': 4, 'comment': 'Buen servicio'},
            {'client': 'Luis M.', 'rating': 5, 'comment': 'Llegó en perfecto estado'},
          ];
          _isLoading = false;
        });
        return;
...
// ----------------------------------------------------------------------------
// PANTALLA DE ESCÁNER DE SMARTWATCH QR
// ----------------------------------------------------------------------------
class _WatchScannerScreen extends StatefulWidget {
  final Function(String) onScanned;
  const _WatchScannerScreen({required this.onScanned});

  @override
  State<_WatchScannerScreen> createState() => _WatchScannerScreenState();
}

class _WatchScannerScreenState extends State<_WatchScannerScreen> {
  bool _hasScanned = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.black,
        foregroundColor: Colors.white,
        title: const Text('Escanear QR del Smartwatch', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        elevation: 0,
      ),
      body: Stack(
        children: [
          ...
          // Guía de encuadre cuadrada para el código QR del reloj
          Center(
            child: Container(
              width: 220,
              height: 220,
              decoration: BoxDecoration(
                border: Border.all(color: const Color(0xFF10B981), width: 3), // verde esmeralda
                borderRadius: BorderRadius.circular(20),
              ),
            ),
          ),
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: const Text(
                    'Encuadra el código QR en la pantalla de tu reloj',
